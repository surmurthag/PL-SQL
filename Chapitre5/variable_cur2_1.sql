...
-- ouverture du curseur
   OPEN cur_emp FOR
                   ...
                   WHERE NUMDEPT IN
                               ...
                               FROM DEPARTEMENT
                               WHERE DNOM='FINANCE' );
   DBMS_OUTPUT.PUT_LINE(' Les employés qui travaillent dans le département finance sont : '); 
   LOOP
   ...
   DBMS_OUTPUT.PUT_LINE(' Les employés qui travaillent dans le département technique sont : '); 
   LOOP
      FETCH cur_emp INTO v_enr_emp;
   EXIT when cur_emp%NOTFOUND;
      DBMS_OUTPUT.PUT_LINE('-- '|| v_enr_emp.enom);
